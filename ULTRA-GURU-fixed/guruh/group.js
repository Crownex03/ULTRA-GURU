...
gmd(
  {
    ...
    react: "⚠️",
    category: "group",
    description: "Warn a group member. After 3 warns they are kicked. Usage: .warn @user",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isBotAdmin, isAdmin, isSuperAdmin, isSuperUser, q, mentionedJid, quotedUser, sender, mek, botPrefix } = conText;
    const { getGroupSetting, setGroupSetting } = require("../guru/database/groupSettings");
    ...
    let targetJid = mentionedJid?.[0] || quotedUser;
    if (!targetJid && q) { const n = q.replace(/\D/g, ""); if (n.length >= 10) targetJid = n + "@s.whatsapp.net"; }
    if (!targetJid) return reply(`❌ Tag or reply to a user!\nUsage: ${botPrefix}warn @user`);
    if (!targetJid.includes("@")) targetJid += "@s.whatsapp.net";
    const num = targetJid.split("@")[0];

    if (targetJid === sender) return reply("❌ You can't warn yourself!");

    const warnKey = `WARN_${num}`;
    const rawCount = await getGroupSetting(from, warnKey);
    let count = parseInt(rawCount) || 0;
    count++;

    const maxWarns = parseInt(await getGroupSetting(from, "WARN_LIMIT")) || 3;

    if (count >= maxWarns) {
      await setGroupSetting(from, warnKey, "0");
      try { await Gifted.groupParticipantsUpdate(from, [targetJid], "remove"); } catch (_) {}
      await react("🚫");
      return reply(`⚠️ @${num} has reached *${maxWarns} warnings* and has been *kicked*!`, { mentions: [targetJid] });
    }

    await setGroupSetting(from, warnKey, String(count));
    await react("⚠️");
    return reply(
      `⚠️ *WARNING ${count}/${maxWarns}*\n\n` +
      `@${num} has been warned!\n` +
      `They will be kicked at ${maxWarns} warnings.\n\n` +
      `_Use ${botPrefix}resetwarn @user to clear warnings_`,
      { mentions: [targetJid] }
    );
  }
);
...
gmd(
  {
    ...
    react: "🔄",
    category: "group",
    description: "Reset warn count for a user. Usage: .resetwarn @user",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isAdmin, isSuperAdmin, isSuperUser, mentionedJid, quotedUser, q, sender, botPrefix } = conText;
    const { setGroupSetting } = require("../guru/database/groupSettings");
    ...
    await setGroupSetting(from, `WARN_${num}`, "0");
    await react("✅");
    return reply(`✅ Warnings cleared for @${num}!`, { mentions: [targetJid] });
  }
);
...
gmd(
  {
    ...
    aliases: ["warnlimit", "setwarn"],
    react: "⚙️",
    category: "group",
    description: "Set max warnings before kick. Default: 3. Usage: .setwarnlimit 5",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isAdmin, isSuperAdmin, isSuperUser, q, botPrefix } = conText;
    const { setGroupSetting, getGroupSetting } = require("../guru/database/groupSettings");

    if (!isGroup) return reply("❌ This command only works in groups!");
    if (!isAdmin && !isSuperAdmin && !isSuperUser) return reply("❌ Admin/Owner Only Command!");

    if (!q) {
      const current = await getGroupSetting(from, "WARN_LIMIT") || "3";
      return reply(`⚙️ *Warn Limit*\nCurrent: *${current}* warnings before kick\n\nUsage: ${botPrefix}setwarnlimit <number>`);
    }

    const limit = parseInt(q.trim());
    if (isNaN(limit) || limit < 1 || limit > 20) return reply("❌ Please provide a number between 1 and 20.");

    await setGroupSetting(from, "WARN_LIMIT", String(limit));
    await react("✅");
    return reply(`✅ Warn limit set to *${limit}*. Users will be kicked after ${limit} warnings.`);
  }
);
...
gmd(
  {
    pattern: "warncount",
    aliases: ["checkwarn", "mywarns"],
    react: "📊",
    category: "group",
    description: "Check warn count for a user. Usage: .warncount @user",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, mentionedJid, quotedUser, q, sender } = conText;
    const { getGroupSetting } = require("../guru/database/groupSettings");

    if (!isGroup) return reply("❌ This command only works in groups!");

    let targetJid = mentionedJid?.[0] || quotedUser || sender;
    if (!targetJid.includes("@")) targetJid += "@s.whatsapp.net";
    const num = targetJid.split("@")[0];

    const count = parseInt(await getGroupSetting(from, `WARN_${num}`)) || 0;
    const maxWarns = parseInt(await getGroupSetting(from, "WARN_LIMIT")) || 3;

    await react("📊");
    return reply(`📊 *Warn Status*\n\n@${num}: *${count}/${maxWarns}* warnings`, { mentions: [targetJid] });
  }
);
...
gmd(
  {
    pattern: "mute",
    react: "🔇",
    aliases: ["mutegc", "mutemember"],
    category: "group",
    description: "Mute a group member (prevent them from sending messages). Usage: .mute @user",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isBotAdmin, isAdmin, isSuperAdmin, isSuperUser, mentionedJid, quotedUser, q, sender } = conText;
    const { setGroupSetting } = require("../guru/database/groupSettings");
    ...
    await setGroupSetting(from, `MUTED_${num}`, "true");
    await react("🔇");
    return reply(`🔇 @${num} has been *muted*! Their messages will be deleted.`, { mentions: [targetJid] });
  }
);
...
gmd(
  {
    pattern: "unmutegc",
    aliases: ["unmutegcmember", "unmember"],
    react: "🔊",
    category: "group",
    description: "Unmute a group member. Usage: .unmutegc @user",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isBotAdmin, isAdmin, isSuperAdmin, isSuperUser, mentionedJid, quotedUser, q } = conText;
    const { setGroupSetting } = require("../guru/database/groupSettings");
    ...
    await setGroupSetting(from, `MUTED_${num}`, "false");
    await react("🔊");
    return reply(`🔊 @${num} has been *unmuted*!`, { mentions: [targetJid] });
  }
);
...
gmd(
  {
    pattern: "setgcicon",
    aliases: ["groupicon", "setgroupicon", "gcicon"],
    react: "🖼️",
    category: "group",
    description: "Set the group profile picture. Reply to an image with this command.",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isBotAdmin, isAdmin, isSuperAdmin, isSuperUser, quoted, quotedMsg, mek } = conText;
    const { downloadMediaMessage } = require("gifted-baileys");
    ...
    if (!quoted?.imageMessage && !quotedMsg?.imageMessage) {
      return reply("❌ Reply to an image to set it as the group icon!");
    }
    ...
    try {
      ...
      await react("❌");
      return reply(`❌ Failed to update group icon: ${error.message}`);
    }
  }
);
...
gmd(
  ...
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isBotAdmin, isSuperUser, sender, mek } = conText;
    ...
    try {
      ...
      const toRemove = meta.participants
        .filter(p => !p.admin && p.id !== botJid && p.id !== sender)
        .map(p => p.id);

      if (toRemove.length === 0) return reply("📭 No regular members to kick!");

      await reply(`⚠️ Kicking *${toRemove.length}* members...`);
      const batchSize = 5;
      for (let i = 0; i < toRemove.length; i += batchSize) {
        const batch = toRemove.slice(i, i + batchSize);
        try { await Gifted.groupParticipantsUpdate(from, batch, "remove"); } catch (_) {}
        await new Promise(r => setTimeout(r, 1000));
      }
      await react("✅");
      return reply(`✅ Successfully kicked *${toRemove.length}* members!`);
    } catch (error) {
      await react("❌");
      return reply(`❌ Failed to kick all: ${error.message}`);
    }
  }
);
...
gmd(
  {
    pattern: "softban",
    aliases: ["tempban", "kickadd"],
    react: "🔨",
    category: "group",
    description: "Soft-ban: kicks and re-adds a user (warning punishment). Usage: .softban @user",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isBotAdmin, isAdmin, isSuperAdmin, isSuperUser, mentionedJid, quotedUser, q, sender } = conText;
    ...
    try {
      await reply(`🔨 Soft-banning @${num}...`, { mentions: [targetJid] });
      await Gifted.groupParticipantsUpdate(from, [targetJid], "remove");
      await new Promise(r => setTimeout(r, 2000));
      await Gifted.groupParticipantsUpdate(from, [targetJid], "add");
      await react("✅");
      return reply(`✅ @${num} has been *soft-banned* (kicked & re-added).`, { mentions: [targetJid] });
    } catch (error) {
      await react("❌");
      return reply(`❌ Soft-ban failed: ${error.message}`);
    }
  }
);
...
gmd(
  {
    ...
    react: "📋",
    category: "group",
    description: "Show quick group info summary",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, mek, botName, newsletterJid } = conText;
    ...
    try {
      ...
      const msg =
        ...
        `🔒 *Restricted:* ${meta.announce ? "Yes" : "No"}\n` +
        `✋ *Approval needed:* ${meta.joinApprovalMode ? "Yes" : "No"}\n` +
        `📅 *Created:* ${new Date(meta.creation * 1000).toLocaleDateString()}\n` +
        `🔗 *Invite Link:* https://chat.whatsapp.com/${inviteCode}`;

      await Gifted.sendMessage(from, { text: msg, contextInfo: { forwardedNewsletterMessageInfo: { newsletterJid, newsletterName: botName, serverMessageId: 0 } } }, { quoted: mek });
      await react("✅");
    } catch (error) {
      await react("❌");
...
gmd(
  {
    pattern: "listadmins",
    aliases: ["admins", "gcadmins", "groupadmins"],
    react: "👑",
    category: "group",
  ...
  async (from, Gifted, conText) => {
    ...
    try {
      ...
      if (superAdmins.length + admins.length === 0) return reply("❌ No admins found!");

      const mentions = [...superAdmins, ...admins].map(p => p.id);
      let msg = `👑 *GROUP ADMINS*\n${"─".repeat(25)}\n\n`;
      superAdmins.forEach(p => { msg += `👑 Owner: @${p.id.split("@")[0]}\n`; });
      admins.forEach(p => { msg += `👮 Admin: @${p.id.split("@")[0]}\n`; });
      msg += `\n📊 Total: *${mentions.length}* admin(s)`;

      await Gifted.sendMessage(from, { text: msg, mentions }, { quoted: mek });
      await react("✅");
    } catch (error) {
      await react("❌");
...
gmd(
  {
    pattern: "listmembers",
    aliases: ["members", "gcmembers", "groupmembers"],
    react: "👥",
    category: "group",
    description: "List all members in the group",
  },
  async (from, Gifted, conText) => {
    const { reply, react, isGroup, isAdmin, isSuperAdmin, isSuperUser, mek } = conText;
...
gmd(
  {
    pattern: "jointoapproval",
    aliases: ["approvalmode", "setapproval"],
    react: "🔐",
    category: "group",
    description: "Toggle join approval mode. Usage: .jointoapproval on/off",
  },
  async (from, Gifted, conText) => {
    ...
    const action = args[0]?.toLowerCase();
    if (!action || !["on", "off"].includes(action)) {
      return reply(`🔐 *Join Approval Mode*\n\nUsage:\n${botPrefix}jointoapproval on - Require approval to join\n${botPrefix}jointoapproval off - Anyone can join`);
    }

    try {
      const meta = await Gifted.groupMetadata(from);
      await Gifted.groupJoinApprovalMode(from, action === "on" ? "on" : "off");
      await react("✅");
      return reply(`✅ Join approval mode is now *${action.toUpperCase()}*!`);
    } catch (error) {
      await react("❌");
      return reply(`❌ Failed: ${error.message}`);
    }
