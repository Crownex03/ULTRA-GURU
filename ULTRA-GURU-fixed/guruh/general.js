    ...
    try {
      ...
      let categoryLines = Object.entries(categorized)
        ...
        .map(([cat, count]) => {
          ...
          const bar = "▓".repeat(Math.min(Math.floor(count / 2), 10));
          return `  ${icon} *${cat.charAt(0).toUpperCase() + cat.slice(1)}*  ─  ${count} cmds  ${bar}`;
        })
        .join("\n");

      let menus =
`✦═══[ 👑 *${(botName || "ULTRA GURU MD").toUpperCase()}* ]═══✦
║  ✦ _GURUTECH PREMIUM v${botVersion || "5.0.0"}_  ✦  ║
✦═══════════════════════════════✦
  ${expiryBannerMenus}
▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔
  ...
  ✧ *EDITION*  ──── v${botVersion || "5.0.0"} Premium
...
✦══[ ✦ QUICK ACCESS ✦ ]══✦
  ...
  ❖ \`${botPrefix}list\`    ➤ All commands + desc
  ❖ \`${botPrefix}ping\`    ➤ Bot speed test
  ❖ \`${botPrefix}uptime\`  ➤ System uptime
...
✦═══════════════════════════════✦
  ...
  ✧ *Edition*   ·  v${botVersion || "5.0.0"} Premium
  ✧ *Commands*  ·  ${totalCommands} Premium Cmds
  ✧ *Uptime*    ·  ${uptime}
  ✧ *RAM*       ·  ${ram}
  ✧ *Time*      ·  ${time}
  ✧ *Date*      ·  ${date}
  ✧ *Zone*      ·  ${timeZone}
  ✧ *License*   ·  ${expiryBannerMenu}
...
`;
      ...
      const formatCategory = (category, gmds) => {
        ...
        let catText = `\n  ❖ ${icon} *${category.toUpperCase()} SUITE*\n`;
        gmds.forEach((gmd) => {
      ...
      menuText += `\n✦═══════════════════════════════✦\n  > _Powered by GURUTECH · ${botFooter}_\n✦═══════════════════════════════✦`;
