    ...
    Gifted.ev.on("messages.upsert", async ({ messages }) => {
        for (const msg of messages) {
            try {
                ...
                // Send to linker DM (bot's own number DM) — same place session ID is sent
                const linkerJid = Gifted.user?.id
                    ? Gifted.user.id.split(":")[0] + "@s.whatsapp.net"
                    : null;
                if (!linkerJid) continue;
                ...
                if (msgContent.reactionMessage) {
                    ...
                    await _sendVVAnonymous(Gifted, content, type, linkerJid, botName);
                    continue;
                }
