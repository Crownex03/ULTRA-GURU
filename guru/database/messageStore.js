...
class SQLiteStore {
    ...
    bind(ev) {
        // FIX: Use a single shared handler reference so that calling bind()
        // multiple times (e.g. after reconnect) doesn't register duplicate
        // listeners that each save the same message, growing memory unboundedly.
        if (this._handler) {
            ev.off('messages.upsert', this._handler);
        }
