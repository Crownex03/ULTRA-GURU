...
// FIX: Cache in-flight metadata fetches to prevent duplicate concurrent API calls
// for the same group (race condition under heavy message load)
...
const getGroupMetadata = async (Gifted, jid) => {
    ...
    try {
        ...
        // FIX: Deduplicate concurrent fetches for the same group JID.
        // Without this, a burst of messages in one group triggers N parallel
        // Gifted.groupMetadata() calls which saturates the WA socket and
        // causes "hanging" / missed messages.
        if (_pendingFetches.has(jid)) {
            return _pendingFetches.get(jid);
        }
